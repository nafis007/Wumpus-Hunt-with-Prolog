...
generateNRandomMoves(InitialDirection,N,X,Y,L,MList) :- 
  N > 0, 
  ...
  generateNRandomMoves(Move,NewN,X1,Y1,NewL,NewMList), 
  append([(Move,X1,Y1)],NewL,L),
  append([Move],NewMList,MList).
...
getRandomMovesList(Energy,Direction,XS,YS,L) :- 
  ...
  getAssumedValidPath(RandomDirection,XS,YS,Energy,RemainingEnergy,L0,L1),
  %reverse(L0,[(D,XC,YC)|_]),
  extractLastPosition(L0,D,XC,YC),
  getRandomMovesList(RemainingEnergy,D,XC,YC,NewL), 
  append(L1,NewL,L));
  
  (Energy >= 6, EnergyAfterShoot is (Energy-6), 
  getRandomDirection(Direction,RandomDirection), 
  getAssumedValidPath(RandomDirection,XS,YS,EnergyAfterShoot,
                     RemainingEnergy,L0,L1), 
  %reverse(L0,[(D,XC,YC)|_]),	
  extractLastPosition(L0,D,XC,YC),  
  getRandomMovesList(RemainingEnergy,D,XC,YC,NewL),
...
extractLastPosition([],_,_,_).  
extractLastPosition(L,D,XC,YC) :-
  reverse(L,[(D,XC,YC)|_]).
...
getAssumedValidPath(Direction,X,Y,Energy,RemainingEnergy,TupleList,MoveList) :- 
  getMazeDimenstion(Direction,Dimension), 
  Depth is min(Energy,Dimension),
  generateNRandomMoves(Direction,Depth,X,Y,TupleList,MoveList), 
