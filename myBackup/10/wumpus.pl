...
generateNRandomMoves(InitialDirection,N,X,Y,L) :- 
  ...
  NewN is N-1,
  generateNRandomMoves(InitialDirection,NewN,X1,Y1,NewL), 
  append([Move],NewL,L).
  
%generateNRandomMoves(Move,N,X,Y,[]) :-
%  N > 0, not(isNewMoveValid(Move,X,Y)).
  
generateNRandomMoves(_,_,_,_,[]).
  

findRemainingMazeDimension(Direction,X,Y,PartialDimension) :- 
  (Direction = east, mazeDimension(_,C), PartialDimension is C-X);
  (Direction = west, mazeDimension(_,_), PartialDimension is X-1);
  (Direction = north, mazeDimension(_,_), PartialDimension is Y-1);
  (Direction = south, mazeDimension(R,_), PartialDimension is R-Y).
