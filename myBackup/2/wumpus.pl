...
isSafe(X,Y) :- maze(X,Y,'.') ; maze(X,Y,"smell"); maze(X,Y,"stench").
...
isDiscoverable(X,Y, FromX, FromY) :- 
  maze(X,Y,"unknown"), isSafe(FromX,FromY), adjacentPosition(_,X,Y,FromX,FromY).
...
initialState(NR, NC, XS, YS, state(XS,YS)) :- 
	...
	INF is NR*NC+1,
	foreach(between(1,NC,X), foreach(between(1,NR,Y), (assert(maze(X,Y,"unknown")),assert(dist(X,Y,north,INF)),assert(dist(X,Y,south,INF)),assert(dist(X,Y,west,INF)),assert(dist(X,Y,east,INF))  ))),
...
generate_shoot_plan(XS,YS, D, L1) :- (maze(X,Y,"unknown") ; maze(X,Y,'W')), wumpusPosition(X,Y)
...
processMove(N,Move, wall, X0,Y0,X0,Y0) :- member(D,DirectionList), adjacentPosition(Move,X0,Y0,X1,Y1), updateWall(X1,Y1), updateDist(X1,Y1,Move,N).
processMove(N,Move, empty, X0,Y0,X1,Y1) :- member(D,DirectionList), adjacentPosition(Move,X0,Y0,X1,Y1), retract(maze(X1,Y1,_)), assert(maze(X1,Y1,'.')), updateDist(X1,Y1,Move,N).
processMove(N,Move, wumpus, X0,Y0,X0,Y0) :- member(D,DirectionList), adjacentPosition(Move,X0,Y0,X1,Y1), retract(maze(X1,Y1,_)), assert(maze(X1,Y1,'W')), updateDist(X1,Y1,Move,N).
processMove(N,Move, pit, X0,Y0,X0,Y0) :- member(D,DirectionList), adjacentPosition(Move,X0,Y0,X1,Y1), retract(maze(X1,Y1,_)), assert(maze(X1,Y1,'P')), updateDist(X1,Y1,Move,N). 
processMove(N,Move, stench, X0,Y0,X1,Y1) :- member(D,DirectionList), adjacentPosition(Move,X0,Y0,X1,Y1), retract(maze(X1,Y1,_)), assert(maze(X1,Y1, "stench")), updateDist(X1,Y1,Move,N).
processMove(N,Move, smell, X0,Y0,X1,Y1) :- member(D,DirectionList), adjacentPosition(Move,X0,Y0,X1,Y1), retract(maze(X1,Y1,_)), assert(maze(X1,Y1, "smell")), updateDist(X1,Y1,Move,N).
processMove(N,Move, damp, X0,Y0,X1,Y1) :- member(D,DirectionList), adjacentPosition(Move,X0,Y0,X1,Y1), retract(maze(X1,Y1,_)), assert(maze(X1,Y1, '.')), updateDist(X1,Y1,Move,N). 
...
wumpusPosition(X,Y) :- maze(X,Y,"unknown"), allSmellValid(X,Y), allStenchValid(X,Y), allEmptyValid(X,Y).
...
allEmptyValid(X,Y) :- maze(XS,YS,'.'), getManhattanDistance(X,Y,XS,YS,Dst), Dst =< 3, !, fail.
allEmptyValid(_,_).
