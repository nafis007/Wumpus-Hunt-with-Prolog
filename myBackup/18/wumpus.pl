...
% This predicate is called continuously by the testing system, until
...
% updates the "maze" facts and "cellDistance" facts which are all the 
% information about the maze; based on the Instruction, Feedback and
% distance Dist (for updating distance).
% Position (X0,Y0) is the previous position before executing the instruction.
% Position (X1,Y1) is the final position after executing the instruction, for
% which the robot gets the feedback. We update the facts based on those 
% feedbacks. We also do not update any distance for the cells where, 
% the robot cannot go(wall) or is unsafe (pit and wumpus), since they
% will never be in the finally planned path.
...
% add shoot to Plan. Ofcourse, the assumed wumpus position (X,Y) need to
...
% to add any direction in the plan, as it will just shoot in the last 
% moved(to (XShoot,YShoot)) direction.
...
% facts where necessary. So, it always initializes with 1 when this predicate
